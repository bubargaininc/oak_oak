	...
	public function show($style = null){
		...
		switch ($this->style) {
			...
			case '5':
				...
				break;
			  case '6':
			    $this->left_current_html = '<a class="ins on">';
		        $this->right_current_html = '</a>';
				if ($this->getNowPage() == 1){
					//$html_page .= '<a>'.$this->left_inside_a_html.$this->pre_home.$this->right_inside_a_html.'</a>';
					$html_page .= '<a>'.$this->left_inside_a_html.$this->pre_page.$this->right_inside_a_html.'</a>';
				}else {
					$html_page .= '<a class="demo" href="'. $this->setShopPseudoStaticPageUrl('1') .'">'.$this->left_inside_a_html.$this->pre_home.$this->right_inside_a_html.'</a>';
					$html_page .= '<a class="demo" href="'. $this->setShopPseudoStaticPageUrl($this->getNowPage()-1) .'">'.$this->left_inside_a_html.$this->pre_page.$this->right_inside_a_html.'</a>';
				}
				$html_page .= $this->getNowBar();
				if ($this->getNowPage() == $this->getTotalPage() || $this->getTotalPage() == 0){
					$html_page .= '<a>'.$this->left_inside_a_html.$this->next_page.$this->right_inside_a_html.'</a>';
					//$html_page .= '<a>'.$this->left_inside_a_html.$this->pre_last.$this->right_inside_a_html.'</a>';
				}else {
					$html_page .= '<a class="demo" href="'. $this->setShopPseudoStaticPageUrl($this->getNowPage()+1) .'">'.$this->left_inside_a_html.$this->next_page.$this->right_inside_a_html.'</a>';
					$html_page .= '<a class="demo" href="'. $this->setShopPseudoStaticPageUrl($this->getTotalPage()) .'">'.$this->left_inside_a_html.$this->pre_last.$this->right_inside_a_html.'</a>';
				}
		...
		if ($this->getNowPage() == $page){
		    ...
		    if (in_array($this->style, array(4,5))) {     // 商城伪静态使用
		        $result = $this->left_html."<a class='demo' href='". $this->setShopPseudoStaticPageUrl($page) ."'>".$this->left_inside_a_html.$page_name.$this->right_inside_a_html."</a>".$this->right_html;
                 ...
                 if(in_array($this->style, array(6))){
						 $result = "<a class='ins' href='". $this->page_url . $page ."'>".$this->left_inside_a_html.$page_name.$this->right_inside_a_html."</a>";
                 }else{
                 	   // 普通分页使用
