...
class memberControl extends SystemControl{
 ...
 /**
   ...
   */ 
   ...
   $model_member = Model('goods_sheji');
	/**
		 * 检索条件
		 */
   ...
   public function sheji_addOp(){
   	$lang	= Language::getLangContent();
		...
		if (chksubmit()){
			...
			array("input"=>$_POST["sheji_name"], "require"=>"true", "message"=>'请填写设计师名称'),
			);
			...
			if ($error != ''){
				...
				$insert_array = array();
					$insert_array['sheji_name']			= trim($_POST['sheji_name']);
				$insert_array['sheji_desc']				= $_POST['sheji_desc'];
				$insert_array['sheji_brand_desc']		= $_POST['sheji_brand_desc'];
				$insert_array['sheji_company'] 			= trim($_POST['sheji_company']);
				$insert_array['sheji_count'] 			= trim($_POST['sheji_count']);
				$insert_array['sheji_address']			= trim($_POST['sheji_address']);
				$insert_array['sheji_order'] 			= trim($_POST['sheji_order']);
				$insert_array['sheji_sex'] 		= trim($_POST['sheji_sex']);
				$insert_array['sheji_time'] 		= trim($_POST['sheji_time']);

                //默认允许举报商品
                $insert_array['inform_allow'] 	= '1';
				if (!empty($_POST['sheji_avatar'])){
					$insert_array['sheji_avatar'] = trim($_POST['sheji_avatar']);
				}
				...
				if ($result){
					$url = array(
					array(
					'url'=>'index.php?act=member&op=sheji',
					'msg'=>$lang['member_add_back_to_list'],
					),
					array(
					'url'=>'index.php?act=member&op=sheji_add',
		...
		Tpl::showpage('member.sheji_add');
   }

	/**
	 * 设计师修改
	 */
	public function sheji_editOp(){
		$lang	= Language::getLangContent();
		$model_member = Model('goods_sheji');
		/**
		 * 保存
		...
		if (chksubmit()){
			...
			$obj_validate = new Validate();
			$obj_validate->validateparam = array(
			array("input"=>$_POST["sheji_name"], "require"=>"true"),
			);
			...
			if ($error != ''){
				...
				$update_array = array();
				$update_array['sheji_id']			= intval($_POST['sheji_id']);
				$update_array['sheji_name']			= trim($_POST['sheji_name']);
				$update_array['sheji_desc']				= $_POST['sheji_desc'];
				$update_array['sheji_brand_desc']		= $_POST['sheji_brand_desc'];
				$update_array['sheji_company'] 			= trim($_POST['sheji_company']);
				$update_array['sheji_count'] 			= trim($_POST['sheji_count']);
				$update_array['sheji_address']			= trim($_POST['sheji_address']);
				$update_array['sheji_order'] 			= trim($_POST['sheji_order']);
				$update_array['sheji_sex'] 		= trim($_POST['sheji_sex']);
				$update_array['sheji_time'] 		= trim($_POST['sheji_time']);
				//exit();
			/*	if (!empty($_POST['sheji_avatar'])){
					$update_array['sheji_avatar'] = $_POST['sheji_avatar'];
				}
*/
		if (!empty($_FILES['member_logo']['name'])){
				...
				if ($result){
					$update_array['sheji_avatar'] = $upload->file_name;
				}else {
		...
		if (!empty($_FILES['site_logo']['name'])){
				...
				if ($result){
					$update_array['sheji_max_img'] = $upload->file_name;
				}else {
					showMessage($upload->error,'','','error');
				}
			}

		$result = $model_member->updateMember($update_array,intval($_POST['sheji_id']));
				if ($result){
					$url = array(
					array(
					'url'=>'index.php?act=member&op=sheji',
					...
					array(
					'url'=>'index.php?act=member&op=sheji_edit&sheji_id='.intval($_POST['sheji_id']),
		...
		Tpl::showpage('member.sheji_edit');
	}
	 ...
	 * 删除设计师
	 */
	public function sheji_delOp(){
		$lang	= Language::getLangContent();
		$model_brand = Model('goods_sheji');
		if (intval($_GET['del_sheji_id']) > 0){
			$model_brand->del(intval($_GET['del_sheji_id']));
			showMessage($lang['nc_common_del_succ'],'index.php?act=member&op=sheji');
		}else {
			showMessage($lang['nc_common_del_fail'],'index.php?act=member&op=sheji');
		}
	}
