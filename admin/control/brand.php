	...
	public function brand_addOp(){
		...
		if (chksubmit()){
			...
			if ($error != ''){
				...
				if ($result){
					$insert_array['brand_pic_sm'] = $upload->file_name;
				}else {
			...
			if (!empty($_FILES['member_logo']['name'])){
				...
				if ($result){
					$insert_array['brand_pic'] = $upload->file_name;
				}else {
					showMessage($upload->error,'','','error');
				}
			}
			if (!empty($_FILES['seller_center_logo']['name'])){
				...
				if ($result){
					$insert_array['brand_pic_pc'] = $upload->file_name;
				}else {
					showMessage($upload->error,'','','error');
				}
			}
				...
				$insert_array['brand_about'] = intval($_POST['brand_about']);
				$insert_array['brand_sort'] = intval($_POST['brand_sort']);
				$insert_array['brand_count'] = intval($_POST['brand_count']);
		...
		$gc_count=Model()->table('count')->where()->order('count_id desc')->select();
		Tpl::output('gc_count', $gc_count);
		// 一级商品分类
    ...
    $upload		= new UploadFile();
		...
		if (chksubmit()){
			...
			if ($error != ''){
				...
				$update_array['brand_count'] = trim($_POST['brand_count']);
				...
				$result = $upload->upfile('site_logo');
				if ($result){
					$update_array['brand_pic_sm'] = $upload->file_name;
				}else {
					showMessage($upload->error,'','','error');
				}
			}
			if (!empty($_FILES['member_logo']['name'])){
				...
				$result = $upload->upfile('member_logo');
				if ($result){
					$update_array['brand_pic'] = $upload->file_name;
				}else {
					showMessage($upload->error,'','','error');
				}
			}
			if (!empty($_FILES['seller_center_logo']['name'])){
				...
				$result = $upload->upfile('seller_center_logo');
				if ($result){
					$update_array['brand_pic_pc'] = $upload->file_name;
				}else {
					showMessage($upload->error,'','','error');
				}
			}
				...
				$update_array['brand_about'] =$_POST['brand_about'];
