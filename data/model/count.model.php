...
class countModel extends Model{
     ...
     */
    public function addGoods($insert, $table = "count") {
     ...
     */
    public function addGoodsAll($insert, $table = 'count') {
     ...
     */
    public function addCount($param) {
        if(empty($param)) {
            return false;
        }
        $member_info    = array();
        $member_info['count_id']            = $param['count_id'];
        $member_info['count_name']          = $param['count_name'];
         $member_info['count_images']            = $param['count_images'];
         $member_info['count_sort']          = $param['count_sort'];
        $result = Db::insert('count',$member_info);
        if($result) {
            return Db::getLastId();
        } else {
            return false;
      ...
      /*
         * 查询国籍
         */
        public function checkCount($type = 1){
            //1:德国 2:法国 3：英国 4:荷兰 5:意大利
            switch ($type){
                case 1:
                    $result='/img/ae-flag.png';
                     break;
                case 2:   
                    $result='/img/faguo.png';
                    break;
                case 3:
                    $result='/img/yinguo.png';
                    break;
                case 4:
                    $result='/img/helan.png';
                     break;
                case 5:
                    $result='/img/yidali.png';
                    break;
                default :
                   $result='/img/ae-flag.png';
                   break;
     ...
     * @return array
     */
    public function getCountInfo($condition, $field = '*') {
     ...
     * @param string $order
     */
    public function getList($condition = array(), $field = '*', $page = 0, $order = 'count_sort desc') {
   ...
   //获取名称
    public function getCountname($id){
        $condition='count_id='.$id;
        $sd= $this->field('count_name')->where($condition)->find();
        $data=$sd['count_name'];
        return $data;   
    }
    ...
    public function getCount($id){
       $condition='count_id='.$id;
        $sd= $this->field('count_images')->where($condition)->find();
        $data=$sd['count_images'];
        return $data;   
    }
    ...
    public function del($id){
        if (intval($id) > 0){
            $where = " count_id = '". intval($id) ."'";
            $result = Db::delete('count',$where);
            return $result;
        }else {
            return false;
        }
     ...
     * @return  array 数组格式的返回结果
     */
    public function updateMember($param,$count_id) {
        if(empty($param)) {
            return false;
        }
        $update     = false;
        //得到条件语句
        $condition_str  = "count_id='{$count_id}' ";
        $update     = Db::update('count',$param,$condition_str);
        return $update;
    }
    ...
    public function getGoodsList($condition, $field = '*', $group = '',$order = '', $limit = 0, $page = 0, $lock = false, $count = 0) {
        ...
        return $this->table('count')->field($field)->where($condition)->group($group)->order($order)->limit($limit)->page($page, $count)->lock($lock)->select();
    }
