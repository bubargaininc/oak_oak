...
//   	    $ac_ids	= implode(',',$ac_ids);
		...
		$article_list	= $article_model->getArticleList($condition,'3');
     Tpl::output('article_class',$article_list);  
     Tpl::output('index_sign','about');
   	 Tpl::showpage('article_about');
   }


   //商品规范
 public function guifanOP(){
   	//获取关于欧橡分类文章
   	$article_model	= Model('article');
     Tpl::output('index_sign','about');
   	 Tpl::showpage('article_guifan');
   }
   ...
   public function shangOP(){
	...
	public function showOp(){
		...
		$article_class_model	= Model('article_class');
        //获取类别
        $class_name=$article_class_model->getOneClass($article['ac_id']);
		Tpl::output('class_name',$class_name);       
