...
class show_storeControl extends BaseStoreControl {
	...
	public function indexOp(){
       ...
       //查询卖家信息
        $stores_info=Model('store')->getStoreInfo($condition);
        $member_info= Model('member')->getMemberInfo(array('member_id'=>$stores_info['member_id']));
       Tpl::output('member',$member_info);

        $model_goods = Model('goods'); // 字段
        $fieldstr = "goods_id,goods_commonid,goods_name,goods_jingle,store_id,store_name,goods_price,goods_marketprice,goods_storage,goods_image,goods_freight,goods_salenum,color_id,evaluation_good_star,evaluation_count,goods_count,goods_collect";
		//得到最新12个商品列表
   ...
   /**获取品牌国籍*/
        foreach ($new_goods_list as $key => $value) {
            	  $new_goods_list[$key]['goods_count']=Model('count')->getCount($value['goods_count']);
             }
...
//商店讨论
 ...
 /**获取品牌国籍*/
        foreach ($recommended_goods_list as $key => $value) {
        	  $recommended_goods_list[$key]['goods_count']=Model('count')->getCount($value['goods_count']);
        }
