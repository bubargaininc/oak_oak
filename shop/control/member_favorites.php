...
class member_favoritesControl extends BaseMemberControl{
    ...
    *愿望单收藏
    */
    public function sgoodsOp(){
      	$fav_id = intval($_GET['fid']);
		if ($fav_id <= 0){
			echo json_encode(array('done'=>false,'msg'=>Language::get('favorite_collect_fail','UTF-8')));
			die;
		}
		$favorites_model = Model('yuan');
		//判断是否已经收藏
		$favorites_info = $favorites_model->getOneFavorites(array('goods_id'=>"$fav_id",'member_id'=>"{$_SESSION['member_id']}"));
	...
	public function favoritesbrandOp(){
		...
		if ($result){
			//增加收藏数量
			Model('brand')->editbrand(array('brand_collect' => array('exp', 'brand_collect + 1')), array('brand_id' => $fav_id));
			echo json_encode(array('done'=>true,'msg'=>Language::get('favorite_collect_success','UTF-8')));
			die;
		}else{
			echo json_encode(array('done'=>false,'msg'=>Language::get('favorite_collect_fail','UTF-8')));
