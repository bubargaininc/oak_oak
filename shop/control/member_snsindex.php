...
class member_snsindexControl extends BaseMemberControl {
	...
	private function formatDate($time){
     ...
     public function yuanOp(){
     	$model=Model('yuan');
      	$goods=$model->getGoodsFavoritesList(array('member_id'=>"{$_SESSION['member_id']}"));
        foreach ($goods as $key => $value) {
         		$goods[$key]['goods']=Model('goods')->getGoodsInfo(array('goods_id' => intval($value['goods_id'])), '*');
         	} 
        Tpl::output('goods',$goods);
     	TpL::showpage('member_yuan');
     }
