...
class member_sharemanageControl extends BaseMemberControl {
	...
	public function share_sinaweiboOp(){
		...
		//修改分享bug 2014.7.1 by lxh
		include_once(BASE_DATA_PATH.DS.'api'.DS.'snsapi'.DS.'sinaweibo'.DS.'callback.php');
	...
	public function share_qqweiboOp(){
		...
		//修改分享bug 2014.7.1 by lxh
		include_once(BASE_DATA_PATH.DS.'api'.DS.'snsapi'.DS.'qqweibo'.DS.'callback.php');
		if (!$_SESSION['qqweibo']){
			echo "<script>alert('".Language::get('sharebind_bind_fail')."');</script>";
			echo "<script>window.close();</script>";
			exit;
