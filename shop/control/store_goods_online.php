...
class store_goods_onlineControl extends BaseSellerControl {
    ...
    public function edit_goodsOp() {
        ...
        if ($goods_class ['type_id'] > 0) {
            ...
            Tpl::output('brand_list', $brand_list);
                ...
                //设计师
       $count_array=Model('count')->getList();
        $sheji_array=Model('goods_sheji')->getMemberList();
        Tpl::output('sheji_array', $sheji_array);
        Tpl::output('count_array', $count_array);
    ...
    public function edit_save_goodsOp() {
        ...
        //欧元价
        $update_common['goods_ouprice']    = $_POST['g_ouprice'];


             //欧元连接
             $update_common['goods_oulink']    = $_POST['g_oulink'];
            //选择国家
             $update_common['goods_count']    = $_POST['g_count'];
            //设计师
             $update_common['goods_sheji']    = $_POST['g_sheji'];
        ...
        if ($return) {
            ...
            if (is_array ( $_POST ['spec'] )) {
                foreach ($_POST['spec'] as $value) {
                    ...
                    $insert['goods_count']      =  $update_common['goods_count'];
                    $insert['goods_sheji']      =  $update_common['goods_sheji'];
                    $insert['goods_oulink']      =  $update_common['goods_oulink'];
                    $insert['goods_ouprice']      =  $update_common['goods_ouprice'];
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        switch ($menu_type) {
        	case 'goods_list':
        	    $menu_array = array(
        	       array('menu_key' => 'goods_list', 'menu_name' => 'Sale of goods', 'menu_url' => urlShop('store_goods_online', 'index'))
        	    );
        	    break;
        	case 'edit_detail':
                $menu_array = array(
                    array('menu_key' => 'edit_detail',    'menu_name' => 'Edit product',   'menu_url' => urlShop('store_goods_online', 'edit_goods', array('commonid' => $_GET['commonid'], 'ref_url' => $_GET['ref_url']))),
                    array('menu_key' => 'edit_image',     'menu_name' => 'Edit pictures',     'menu_url' => urlShop('store_goods_online', 'edit_image', array('commonid' => $_GET['commonid'], 'ref_url' => ($_GET['ref_url'] ? $_GET['ref_url'] : getReferer()))))
                );
        	    break;
        	case 'edit_class':
                $menu_array = array(
                    array('menu_key' => 'edit_class',     'menu_name' => 'Selection and classification', 'menu_url' => urlShop('store_goods_online', 'edit_class', array('commonid' => $_GET['commonid'], 'ref_url' => $_GET['ref_url']))),
                    array('menu_key' => 'edit_detail',    'menu_name' => 'Edit product',   'menu_url' => urlShop('store_goods_online', 'edit_goods', array('commonid' => $_GET['commonid'], 'ref_url' => $_GET['ref_url']))),
                    array('menu_key' => 'edit_image',     'menu_name' => 'Edit pictures',     'menu_url' => urlShop('store_goods_online', 'edit_image', array('commonid' => $_GET['commonid'], 'ref_url' => ($_GET['ref_url'] ? $_GET['ref_url'] : getReferer()))))
                );
        	    break;
