...
class goodsControl extends BaseGoodsControl {
    ...
    public function indexOp() {
        ...
        //优先得到推荐商品
        $goods_commend_list = $model_goods->getGoodsOnlineList(array('store_id' => $goods_info['store_id'], 'goods_commend' => 1), '*', 0, 'rand()', 4, 'goods_commonid');
        foreach ($goods_commend_list as $key => $value) {
            $goods_commend_list[$key]['goods_count']=Model('count')->getCount($value['goods_count']);
        }
