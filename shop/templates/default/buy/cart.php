...
<div class="content-about content-order-ok">
        <div class="stepflex">
            <ul>
                <li class="sfone on">STEP1: 查看购物车</li>
                <li class="sftwo">STEP2: 确认订单信息</li>
                <li class="sfthree">STEP3: 提交订单</li>
            </ul>
        </div>
...
<div class="ncc-main">
  <form action="<?php echo urlShop('buy','buy_step1');?>" method="POST" id="form_buy" name="form_buy">
    ...
    <table class="ncc-table-style" nc_type="table_cart">
      ...
      <tbody>
        <tr>
          <th colspan="20"><i class="icon-home"></i><a target="_blank" href="<?php echo urlShop('show_store','index',array('store_id'=>$store_id), $output['store_list'][$store_id]['store_domain']);?>"><?php echo $cart_list[0]['store_name']; ?></a> <span member_id="<?php echo $output['store_list'][$store_id]['member_id'];?>"></span>
        ...
        <tr nc_group="<?php echo $cart_info['cart_id'];?>">
          <td></td>
          <td class="tl" colspan="10"><div class="store-sale"><em>
<i class="icon-gift"></i>
满即送
</em><?php echo implode('<br/>', $output['mansong_rule_list'][$store_id]);?></div></td>
        </tr>
        <?php }?>
        <!-- E mansong list -->
      ...
      <tfoot>
        <tr>
          <td colspan="20"><div class="ncc-all-account"><?php echo $lang['cart_index_goods_sumary'];?>￥<em id="cartTotal"><?php echo $output['cart_totals']; ?></em>元</div></td>
        </tr>
      </tfoot>
    </table>
  </form>
