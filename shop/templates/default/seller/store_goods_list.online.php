...
<table class="ncsc-table-style">
  <thead>
    ...
    <tr>
      ...
      <td colspan="20"><label for="all" ><?php echo $lang['nc_select_all'];?></label>
        ...
        <a href="javascript:void(0);" class="ncsc-btn-mini" nctype="batch" data-param="{url:'<?php echo urlShop('store_goods_online', 'edit_jingle');?>', sign:'jingle'}"><i></i>Set advertising words</a>
        <a href="javascript:void(0);" class="ncsc-btn-mini" nctype="batch" data-param="{url:'<?php echo urlShop('store_goods_online', 'edit_plate');?>', sign:'plate'}"><i></i>Set associated format</a>
      </td>
    </tr>
    <?php } ?>
  </thead>
  <tbody>
    ...
    <tr>
      ...
      <th colspan="20">Platform:<?php echo $val['goods_commonid'];?></th>
    </tr>
    <tr>
      <td class="trigger"><i class="tip icon-plus-sign" nctype="ajaxGoodsList" data-comminid="<?php echo $val['goods_commonid'];?>" title="Click to view the full specifications of the goods; the specifications are too large, please scan the scroll bar in the area."></i></td>
      ...
      <td class="tl"><dl class="goods-name">
          <dt><?php if ($val['goods_commend']) { echo '<span>Recommended</span>';}?><a href="<?php echo urlShop('goods', 'index', array('goods_id' => $output['storage_array'][$val['goods_commonid']]['goods_id']));?>" target="_blank"><?php echo $val['goods_name']; ?></a></dt>
      ...
      <?php } else {?>
        <span class="tip" title="The goods to participate in group purchase activities can not be edited and deleted and so on"><a href="javascript:void(0);" class="btn-orange-current" style="cursor: default;"><i class="icon-lock"></i><p>Lock</p></a></span>
      <?php }?>
      </td>
    </tr>
...
$(function(){
    ...
    $('a[nctype="batch"]').click(function(){
        ...
        if (data_str.sign == 'jingle') {
            ajax_form('ajax_jingle', 'Set advertising words', data_str.url + '&commonid=' + _items + '&inajax=1', '480');
        } else if (data_str.sign == 'plate') {
            ajax_form('ajax_plate', 'Set associated format', data_str.url + '&commonid=' + _items + '&inajax=1', '480');
        }
    });
});
</script>