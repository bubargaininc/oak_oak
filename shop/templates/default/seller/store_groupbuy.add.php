...
<div class="ncsc-form-default">
  <form id="add_form" action="index.php?act=store_groupbuy&op=groupbuy_save" method="post" enctype="multipart/form-data">
    ...
    <dl>
      <dt>Subtitle group purchase<?php echo $lang['nc_colon'];?></dt>
      <dd>
        <input class="w400 text" name="remark" type="text" id="remark" value="" maxlength="30"  />
        <span></span>
        <p class="hint">Group purchase activities: you can enter up to 30 characters</p>
      </dd>
    </dl>
    <dl>
      ...
      <dd>
          ...
          <p class="hint"><?php echo 'Group purchase can not be less than'.date('Y-m-d H:i', $output['groupbuy_start_time']);?></p>
      </dd>
    </dl>
    <dl>
      ...
      <dd>
          ...
          <p class="hint"><?php echo 'The group purchase can not be more than the starting time'.date('Y-m-d H:i', $output['current_groupbuy_quota']['end_time']);?></p>
      </dd>
    </dl>
    <dl>
      ...
      <div class="goods-thumb"><img id="groupbuy_goods_image" src=""/></div>
          ...
          <div class="goods-price">Mall price：￥<span nctype="groupbuy_goods_price"></span></div>
      </div>
      <a href="javascript:void(0);" id="btn_show_search_goods" class="ncsc-btn ncsc-btn-acidblue">Select goods</a>
      ...
      <div id="div_search_goods" class="div-goods-select mt10" style="display: none;">
          <table class="search-form">
              <tr>
                  <th class="w150">
                      <strong>The first step: search for goods in the store</strong>
                  </th>
                  <td class="w160">
                  ...
                  <td class="w70 tc">
                    ...
                    <td class="w10"></td>
                    <td>
                        <p class="hint">Do not enter the name of the direct search will show all the stores in the sale of goods</p>
                    </td>
                </tr>
            </table>
    ...
    <dl>
      <dt><i class="required">*</i>Group buying activity picture<?php echo $lang['nc_colon'];?></dt>
      <dd>
    ...
    <dl>
      <dt>Group buy recommendation<?php echo $lang['nc_colon'];?></dt>
      <dd>
    ...
    <dl>
      ...
      <dd>
        <select id="class_id" name="class_id" class="w80">
          ...
          <option value="<?php echo $groupbuy_class['class_id'];?>"><?php echo $groupbuy_class['class_name'];?></option>
          <?php } ?>
          <?php } ?>
          <?php } ?>
        </select>
    ...
    <dl>
      ...
      <dd>
        <select id="area_id" name="area_id" class="w80">
          <option value="0"><?php echo $lang['text_no_limit'];?></option>
          <?php if(is_array($output['area_list'])) { ?>
          <?php foreach($output['area_list'] as $groupbuy_area) { ?>
          <?php if(intval($groupbuy_area['deep']) === 0) { ?>
          <option value="<?php echo $groupbuy_area['area_id'];?>"><?php echo $groupbuy_area['area_name'];?></option>
          <?php } ?>
          <?php } ?>
          <?php } ?>
        </select>
...
$(document).ready(function(){
    ...
    $("#add_form").validate({
        ...
        messages : {
            ...
            start_time : {
                required : '<i class="icon-exclamation-sign"></i>The group purchase can not be empty',
                greaterThanDate : '<i class="icon-exclamation-sign"></i><?php echo sprintf('The group purchase start time must be greater than{0}',date('Y-m-d H:i',$output['current_groupbuy_quota']['start_time']));?>'
            },
            end_time : {
                required : '<i class="icon-exclamation-sign"></i>Buy end time can not be empty',
                lessThanDate : '<i class="icon-exclamation-sign"></i><?php echo sprintf('The group purchase start time must be less than{0}',date('Y-m-d H:i',$output['current_groupbuy_quota']['end_time']));?>',
                greaterThanStartDate : '<i class="icon-exclamation-sign"></i>The end time must be greater than the start time'
            },
            groupbuy_goods_id: {
                required : '<i class="icon-exclamation-sign"></i><?php echo $lang['group_goods_error'];?>', 
                checkGroupbuyGoods: 'The product has been in the same time.'
            },
            groupbuy_price: { 
            ...
            groupbuy_image: {
                required : '<i class="icon-exclamation-sign"></i>Buy pictures can not be empty'
            }
        }
    });
