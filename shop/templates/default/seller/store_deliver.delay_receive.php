...
<?php if ($output['order_info']) {?>
  ...
  <form id="changeform" method="post" action="index.php?act=store_deliver&op=delay_receive&order_id=<?php echo $output['order_info']['order_id']; ?>">
    ...
    <dl>
      <dt><?php echo 'Late delivery time'.$lang['nc_colon'];?></dt>
      <dd><?php echo date('Y-m-d H:i:s',$output['order_info']['delay_time']);?><br/>If more than the time buyers did not click on the receiving, the system will automatically change to the receiving state </dd>
    </dl>
    <dl>
      <dt><?php echo 'Delay'.$lang['nc_colon'];?></dt>
      <dd>
...
<?php } else { ?>
<p style="line-height:80px;text-align:center">The order does not exist, please check if the parameters are correct.!</p>
<?php } ?>
</div>
