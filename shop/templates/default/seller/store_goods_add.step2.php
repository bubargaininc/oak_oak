  ...
  <form method="post" id="goods_form" action="<?php if ($output['edit_goods_sign']) { echo urlShop('store_goods_online', 'edit_save_goods');} else { echo urlShop('store_goods_add', 'save_goods');}?>">
    ...
    <div class="ncsc-form-goods">
      ...
      <dl>
        ...
        <dd id="gcategory"> <?php echo $output['goods_class']['gc_tag_name'];?> <a class="ncsc-btn" href="<?php if ($output['edit_goods_sign']) { echo urlShop('store_goods_online', 'edit_class', array('commonid' => $output['goods']['goods_commonid'], 'ref_url' => getReferer())); } else { echo urlShop('store_goods_add', 'add_step_one'); }?>">Edit</a>
      ...
      <dl>
        <dt class="required">Advertising words<?php echo $lang['nc_colon'];?></dt>
        <dd>
          <input name="g_jingle" type="text" class="text w400" value="<?php echo $output['goods']['goods_jingle']; ?>" />
          <span></span>
          <p class="hint">Advertising words can not exceed 50 characters</p>
        </dd>
      </dl>
      <dl>
        ...
        <dd nc_type="no_spec">
          ...
          <p class="hint"><?php echo $lang['store_goods_index_store_price_help'];?>，And can not be higher than the market price。<br>This price is the actual sale price of the commodity, if the commodity has the specification, the price shows the lowest price.</p>
        </dd>
      </dl>
      <dl>
        <dt><i class="required">*</i>Market value<?php echo $lang['nc_colon'];?></dt>
        <dd>
          ...
          <p class="hint"><?php echo $lang['store_goods_index_store_price_help'];?>，This price is only for the market reference price, please according to the actual situation seriously fill。</p>
        </dd>
      </dl>
      <dl>
        <dt>Cost price<?php echo $lang['nc_colon'];?></dt>
        <dd>
          ...
          <p class="hint">Prices must be between 0.00~9999999 digital, the price for merchants to the actual cost price of the sale of goods were recorded, non mandatory option, not in the sales front page display。</p>
        </dd>
      </dl>


      <dl>
        <dt>Discount<?php echo $lang['nc_colon'];?></dt>
        <dd>
          ...
          <p class="hint">According to the sales price and the ratio of the market price automatically generated, do not need to edit.</p>
        </dd>
      </dl>


      <dl>
        <dt>Euro price<?php echo $lang['nc_colon'];?></dt>
        <dd>
          <input name="g_ouprice" value="<?php echo $output['goods']['goods_ouprice']; ?>" type="text" class="text w60" /><em class="add-on"><i class="icon-renminbi"></i></em> <span></span>
          <p class="hint">Prices must be between 0.00~9999999 digital, the price for merchants to the actual cost price of the sale of goods were recorded, non mandatory option, not in the sales front page display。</p>
        </dd>
      </dl>

        <dl>
        <dt>Select countries<?php echo $lang['nc_colon'];?></dt>
        <dd>
          
          <select name="g_count">
              <?php foreach ($output['count_array'] as $key => $value) {?>
              <option value="<?php echo $value['count_id']?>"><?php echo $value['count_name']?></option>
              <?php }?>
          </select>
           ...
           <dl>
        <dt>Select Designer<?php echo $lang['nc_colon'];?></dt>
        <dd>
          
          <select name="g_sheji">
          <option value="0">Please choose</option>
              <?php 
              foreach ($output['sheji_array'] as $key => $value) {?>
              <option value="<?php echo $value['sheji_id']?>"><?php echo $value['sheji_name']?></option>
              <?php }?>
          </select>
          <p class="hint"></p>
        </dd>
      ...
      <dl>
        <dt>European parity<?php echo $lang['nc_colon'];?></dt>
        <dd>
          <input name="g_oulink" value="<?php echo $output['goods']['goods_oulinke']; ?>" type="text" class="text w300" />
          <p class="hint">The non mandatory option, will not be displayed in the front desk sales page.</p>
        </dd>
      </dl>
      ...
      <dl nc_type="spec_group_dl_<?php echo $i;?>" nctype="spec_group_dl" class="spec-bg" <?php if($k == '1'){?>spec_img="t"<?php }?>>
        <dt>
          <input name="sp_name[<?php echo $k;?>]" type="text" class="text w60 tip2 tr" title="Custom specification type name, specification value name is not more than 4 words" value="<?php if (isset($output['goods']['spec_name'][$k])) { echo $output['goods']['spec_name'][$k];} else {echo $val['sp_name'];}?>" maxlength="4" nctype="spec_name" data-param="{id:<?php echo $k;?>,name:'<?php echo $val['sp_name'];?>'}"/>
        ...
        <dd <?php if($k == '1'){?>nctype="sp_group_val"<?php }?>>
          <ul class="spec">
            ...
            <li data-param="{gc_id:<?php echo $output['goods_class']['gc_id'];?>,sp_id:<?php echo $k;?>,url:'<?php echo urlShop('store_goods_add', 'ajax_add_spec');?>'}">
              <div nctype="specAdd1"><a href="javascript:void(0);" class="ncsc-btn" nctype="specAdd"><i class="icon-plus"></i>Added value</a></div>
              <div nctype="specAdd2" style="display:none;"><input class="text w60" type="text" placeholder="Added value" maxlength="20"><a href="javascript:void(0);" nctype="specAddSubmit" class="ncsc-btn ncsc-btn-acidblue ml5 mr5">Confirm</a><a href="javascript:void(0);" nctype="specAddCancel" class="ncsc-btn ncsc-btn-orange">Cancel</a></div>
            </li>
          </ul>
        </dd>
      </dl>
            ...
            <div class="goodspic-uplaod">
              ...
              <div class="handle">
                <div class="ncsc-upload-btn"> <a href="javascript:void(0);"><span>
                  ...
                  <p><i class="icon-upload-alt"></i>Picture upload</p>
                  </a> </div>
                <a class="ncsc-btn" nctype="show_image" href="<?php echo urlShop('store_album', 'pic_list', array('item'=>'goods'));?>"><i class="icon-picture"></i>From image space selection</a> <a href="javascript:void(0);" nctype="del_goods_demo" class="ncsc-btn ml5" style="display: none;"><i class="icon-circle-arrow-up"></i>Close album</a></div>
            </div>
          </div>
          <div id="demo"></div>
        </dd>
      ...
      <dl>
        ...
        <dd>
          ...
          <select name="" attr="attr[<?php echo $k;?>][__NC__]" nc_type="attr_select">
            <option value='Unlimited' nc_type='0'>Unlimited</option>
            <?php foreach ($val['value'] as $v){?>
      ...
      <dl>
        ...
        <dd>
          ...
          <div class="hr8"><div class="ncsc-upload-btn"> <a href="javascript:void(0);"><span>
                  ...
                  <p><i class="icon-upload-alt" data_type="0" nctype="add_album_i"></i>Picture upload</p>
                  </a> </div>
                  <a class="ncsc-btn" nctype="show_desc" href="index.php?act=store_album&op=pic_list&item=des"><i class="icon-picture"></i><?php echo $lang['store_goods_album_insert_users_photo'];?></a> <a href="javascript:void(0);" nctype="del_desc" class="ncsc-btn ml5" style="display: none;"><i class=" icon-circle-arrow-up"></i>Close album</a> </p>
          <p id="des_demo"></p>
        </dd>
      </dl>
      <dl>
        <dt>Associated format:</dt>
        <dd> <span class="mr50">
          <label>Top layout</label>
        ...
        <dd>
          <ul class="ncsc-form-radio-list">
            <li>
              ...
              <label for="freight_0">Fixed freight</label>
          ...
          <p class="hint">Freight is set to 0 yuan, the front desk goods will be displayed as free shipping。</p>
        </dd>
      </dl>
      <!--transport info end-->
      <h3>Invoice information</h3>
      <dl>
        <dt>Whether to open value-added tax invoices:</dt>
        <dd>
          <ul class="ncsc-form-radio-list">
            <li>
              <label>
    ...
    <div class="bottom tc hr32">
      <label class="submit-border">
        <input type="submit" class="submit" value="<?php if ($output['edit_goods_sign']) {echo 'Submit';} else {?><?php echo $lang['store_goods_add_next'];?>, upload pictures<?php }?>" />
      </label>
    </div>
  </form>
</div>
...
$(function(){
    ...
    }, '<i class="icon-exclamation-sign"></i>Commodity prices can not be higher than the market price');
    $('#goods_form').validate({
        ...
        messages : {
            ...
            g_jingle : {
                maxlength   : '<i class="icon-exclamation-sign"></i>Advertising can not exceed 50 characters'
            },
            g_price : {
            ...
            g_marketprice : {
                required    : '<i class="icon-exclamation-sign"></i>Please fill in market price',
                number      : '<i class="icon-exclamation-sign"></i>Please fill in the correct price',
                min         : '<i class="icon-exclamation-sign"></i>Please fill in the number between 0.01~9999999',
                max         : '<i class="icon-exclamation-sign"></i>Please fill in the number between 0.01~9999999'
            },
            g_costprice : {
                number      : '<i class="icon-exclamation-sign"></i>Please fill in the correct price',
                min         : '<i class="icon-exclamation-sign"></i>Please fill in the number between 0.01~9999999',
            ...
            image_path : {
                required    : '<i class="icon-exclamation-sign"></i>Please choose Commodity master plan'
            }
        }
    });
